...
- [x] T001 Create directory structure: `css/`, `js/`, `images/`
- [x] T002 [P] Create `css/variables.css` — все CSS custom properties из plan.md (цвета, шрифты, отступы, радиусы)
- [x] T003 [P] Create `css/reset.css` — минимальный reset (box-sizing, margins, list styles, images)
- [x] T004 [P] Create `css/base.css` — body (bg #131313, text #e5e2e1), typography (Noto Serif, Liberation Serif, Manrope через Google Fonts), ссылки, утилиты (.container, .sr-only)
- [x] T005 Create `index.html` — семантический каркас: `<header>`, `<main>` с 6 `<section>`, `<footer>`, подключение CSS/JS, Google Fonts
...
- [x] T006 [P] Create `css/header.css` — навбар: лого «Тихий Двор» (Liberation Serif 24px), телефон, иконки Telegram/WhatsApp, фон rgba(23,23,23,0.8), backdrop-filter blur
- [x] T007 [P] Create `css/footer.css` — футер: фон #0a0a0a, ссылки (Nimbus Sans 14px, #525252), копирайт (#737373), центрирование
- [x] T008 Create `js/navigation.js` — бургер-меню для mobile, smooth scroll к якорям, подсветка активной секции при скролле
- [x] T009 Create `js/lazy-images.js` — IntersectionObserver для lazy loading изображений с placeholder-цветом
- [x] T010 Create `js/main.js` — точка входа: инициализация navigation.js и lazy-images.js
...
- [x] T011 [P] [US1] Create `css/hero.css` — hero-секция: фоновое изображение (двор, гирлянды), overlay с затемнением, заголовок «Кофе и еда в тихом дворе старого Петербурга» (Noto Serif 36px, #e5e2e1), подзаголовок (Manrope 14px, #fcba67), микро-текст «Вход через арку • 2 минуты от Невского»
- [x] T012 [P] [US1] Create `css/cta.css` — финальный CTA-блок: фоновое изображение (вечер, тёплый свет), текст «Зайди на кофе сегодня», кнопки «Посмотреть меню» и «Построить маршрут»
- [x] T013 [US1] Populate hero content in `index.html` — заголовок, подзаголовок, 2 CTA-кнопки, микро-текст, контакты в шапке
- [x] T014 [US1] Populate final CTA content in `index.html` — текст, подтекст, 2 кнопки
- [x] T015 [US1] Add hero background image to `images/hero-bg.webp` + `images/hero-bg.jpg` fallback (стоковое фото: двор-колодец, вечер, гирлянды)
- [x] T016 [US1] Add CTA background image to `images/cta-bg.webp` (стоковое фото: вечерний свет, кофе, люди)
- [x] T017 [US1] Style CTA buttons: primary (#c58a3c bg, #472a00 text, 64px height, 8px radius), secondary (outline или #325037 bg)
...
- [x] T018 [P] [US2] Create `css/menu.css` — секция меню: заголовок H2 (Liberation Serif 30px), золотой разделитель 48×2px, сетка карточек (grid/flex), карточки (bg #202020, radius 8px), hover-эффекты
- [x] T019 [P] [US2] Add stock images for menu: `images/menu-breakfast.webp`, `images/menu-main.webp`, `images/menu-dessert.webp`, `images/menu-coffee.webp`
- [x] T020 [US2] Populate menu section in `index.html` — 4 карточки:
...
- [x] T021 [P] [US3] Create `css/location.css` — секция локация: заголовок H2, блок с адресом (центрированный), карта (iframe или placeholder), контакты (телефон #fcba67, часы #737373), кнопки
- [x] T022 [US3] Populate location section in `index.html`:
...
- [x] T023 [P] [US4] Create `css/why-us.css` — секция «Почему мы»: заголовок H2, 4 пункта с вертикальными бордерами (#514538 30%), H4 (Manrope 18px, #e5e2e1), описания (Manrope 14px, #d5c4b2), фон #1b1b1c
- [x] T024 [US4] Populate why-us section in `index.html`:
...
- [x] T025 [P] Create `css/about.css` — секция about: заголовок H2, текст (Manrope 14px, #d5c4b2), 4 тезиса с иконками, фото дворика и интерьера
- [x] T026 [P] Add stock images: `images/about-1.webp` (дворик, арка), `images/about-2.webp` (интерьер, столики)
- [x] T027 Populate about section in `index.html`:
...
- [x] T028 Create `css/responsive.css` — media queries:
...
- [x] T029 [P] Добавить favicon (иконка кофе/двора)
- [x] T030 [P] Добавить meta-теги: title, description, Open Graph (для шаринга в соцсетях)
- [x] T031 Проверить smooth scroll ко всем якорям (кнопки «Посмотреть меню» → секция меню, «Построить маршрут» → секция локация)
- [x] T032 Проверить все ссылки: Telegram (https://t.me/tihiydvor), WhatsApp (https://wa.me/70000000000), Яндекс.Карты
- [x] T033 Финальная проверка соответствия Figma: цвета, шрифты, отступы, border-radius
- [x] T034 Оптимизировать изображения: WebP с fallback JPG, lazy loading работает
- [x] T035 Проверить в Chrome, Safari, Firefox (desktop + mobile)
